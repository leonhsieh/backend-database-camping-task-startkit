...
select  count(*) as 用戶總數 from "USER" ;
...
insert into "COACH_LINK_SKILL" (coach_id, skill_id)
 values((select id from "COACH" where user_id = (select id from "USER" where email = 'lee2000@hexschooltest.io')),
...
status = '上課中'
where booking_at = '2024-11-24 17:10:25';
...
where cls.skill_id = (select id from "SKILL" s2 where name = '重訓')
...
cp."name" as 組合包方案名稱,
count(*) as 銷售數量
from "CREDIT_PURCHASE" cpur
inner join "CREDIT_PACKAGE" cp on cp.id = cpur.credit_package_id
--資料加入時間為12月，啟用這行看不到資料正常
where cpur.created_at >= '2024-11-01 00:00:00' and cpur.created_at <= '2024-11-30 23:59:59'
group by cp."name" ;
...
where cp.created_at >= '2024-11-01 00:00:00' and cp.created_at <= '2024-11-30 23:59:59';
...
where cb.status != '已取消' 
and cb.created_at >= '2024-11-01 00:00:00' 
